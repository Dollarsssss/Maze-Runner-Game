...
public class TileManager {
    ...
    public TileManager(GamePanel gp) {
        ...
        tile = new Tile[10]; // up size if you have a tiles image more 10 simple now we have 3/10 grass, water, wall
    ...
    public void getTileImage() {
        try { //get image from package tiles water, grass, wall to Array tile
            tile[0] = new Tile();
    ...
    public void loadMap(String filePath) {
        try {
            ...
            InputStream is = getClass().getResourceAsStream(filePath); // import map.txt flie
            BufferedReader br = new BufferedReader(new InputStreamReader(is)); // read map.txt flie in package res
            ...
            while (col < gp.maxWorldCol && row < gp.maxWorldRow) {
                ...
                String line = br.readLine(); // read a valus in map.txt and put to string line

                while (col < gp.maxWorldCol) {

                    String number[] = line.split(" "); //after read map.txt this code will put value to Array number[]   

                    int num = Integer.parseInt(number[col]); // changing String number[] to Integer
    ...
    public void draw(Graphics2D g2) { //X = horizontal and Y = vartical ---- image, X, Y and put ++ 48 pixal to next block
//        g2.drawImage(tile[1].image, 0, 0, gp.tileSize, gp.tileSize, null);    *Old map
...
//        g2.drawImage(tile[1].image, 192, 192, gp.tileSize, gp.tileSize, null);
        ...
        while (worldCol < gp.maxWorldCol && worldRow < gp.maxWorldRow) { //for create
            
            int tileNum = mapTileNum[worldCol][worldRow]; // get value in mapTileNum to tileNum for input to tile[] array for changing block in map 
